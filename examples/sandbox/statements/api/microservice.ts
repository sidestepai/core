...
export const apiMicroservice = defineFunction({
  ...
  stack: [
    s.api.microservice({ as: "result", host: c.text("example"), path: c.text("example"), method: c.text("example"), params: c.obj({}), headers: c.obj({}), timeout: c.int(1), follow_location: c.text("example") }),
  ],
  response: ref("result"),
});
