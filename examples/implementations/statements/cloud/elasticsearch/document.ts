...
export const cloudElasticsearchDocument = defineFunction({
  ...
  stack: [
    s.cloud.elasticsearch.document({ as: "result", key_id: c.text("••••"), access_key: c.text("••••"), region: c.text("example"), index: c.text("example"), base_url: c.text("example"), doc_id: c.text("example"), doc: c.obj({}) }),
  ],
  response: ref("result"),
});
