...
`,
  ...
  it("missing lock without --lock exports unlocked, with the FYI suppressed for now", async () => {
    const entry = writeWorkspace("sayHello");
    await run(["export", entry, "--out", join(dir, "b.json")]);
    // The unlocked-export FYI is intentionally suppressed for now (it fired on
    // every export/deploy). Lock-by-default work will restore a one-time hint.
    expect(stderrText()).not.toContain("Exporting without xano.lock");
    expect(existsSync(join(dir, "xano.lock"))).toBe(false);
