/**
 * Normalizer for the golden-fixture deep-equal (U6). Strips server- and
 * auto-generated keys the engine adds on import (which the SDK never emits) so
 * the authored-field parity can be compared directly.
 *
 * Beyond the plan's enumerated server keys, this also drops `workspace`,
 * `branch`, and `market_item`: in the real fixture these are large
 * server/deploy-target blobs (e.g. `workspace.editBranch`), and deploy-target
 * binding is explicitly deferred from the MVP. Removing them from both sides
 * keeps the comparison focused on authored function logic.
 */
