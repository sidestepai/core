...
describe("verifyBundles", () => {
  ...
  it("treats a workspace key at the engine default as equal to an absent one", () => {
    // These four are emitted BY PRESENCE, so a tree that leaves them out cannot
    // re-export them — but the engine writes all four on every save. Reporting
    // that as a loss (or even as an omission notice) would put four rows on every
    // pull for keys nobody wrote and nobody can act on.
    const result = verifyBundles(
      {
        payload: {
          workspace: {
    ...
    expect(result).toEqual({ ok: true, mismatches: [], omissions: [] });
  });

  it("still reports a workspace key that departs from the engine default", () => {
    // The equivalence is value-driven. A tenant that picked uuid primary keys and
    // a tree that dropped the setting is a real divergence, not tidy-up.
    const result = verifyBundles(
      { payload: { workspace: { name: "ws", defaults: { db_primary_key: "uuid" } } } },
      { payload: { workspace: { name: "ws" } } },
    );
    expect(result.ok).toBe(false);
    expect(result.mismatches[0]).toMatchObject({ payloadKey: "workspace", name: "defaults" });
  });
