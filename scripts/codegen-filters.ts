...
function jsdoc(name: string, spec: FilterSpec | undefined): string {
  const parts: string[] = [];
...
function emit(cat: FilterCatalog): string {
  const factories = cat.names.map((n) => emitFactory(n, cat.specs[n])).join("\n");
  const typedCount = cat.names.filter((n) => cat.specs[n]?.args?.length).length;
  return `/**
...
export const FILTER_SPECS: Readonly<Record<string, FilterSpec>> = ${JSON.stringify(cat.specs)};
