...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "- `s.api.microservice({ host, path, port?, method?, params?, headers?, timeout?, follow_location?, as? })` — in-cluster microservice call (`mvp:microservice_request`); no TLS fields. ONLY `host`+`path` required; the rest default to the engine's values (`GET`/`{}`/`[]`/`10`/`true`), always emitted. Pass the `microservice()` DEF as `host`. `port?` folds into it as `\"name:port\"`: a def exposing ONE `servicePort` resolves automatically, SEVERAL requires it. A raw `\"name:port\"` string works, unvalidated, for instance-level ones.",
