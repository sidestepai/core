...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "- **Addons** enrich returned rows: `db.query`/`get`/`add`/`edit`/`patch` accept `addon: [{ addon, as, input?, output?, children? }]`. `addon` is the target addon (name or def handle); `as` is a dotted `offset.alias` destination on the row; `input` maps addon inputs (bind a parent-row column with `out(col)`); `output` restricts addon columns; `children` nests addons. Each addon's alias (the last `as` segment) is merged onto the row in `InferResponse` as an `unknown`-typed key (the SDK can't type the target addon's columns) — narrow it at the call site. `db.add_or_edit`/`del`/`has`/`truncate` take no `addon`.",
    "",
    "Auth & calls:",
    "",
