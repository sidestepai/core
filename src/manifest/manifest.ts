...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "Typical agent flow: `sandbox deploy ./index.ts` → read the sandbox base URL with",
    "`sandbox details` → build the frontend against it → `sandbox deploy --static ./dist`",
    "to ship API + app together.",
    "",
