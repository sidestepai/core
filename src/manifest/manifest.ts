...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "",
    "- `db.get` **throws** (HTTP 404 Not Found) when no row matches — it does NOT bind `null`. On a hit it binds the **full row**. When a row may be absent, gate with `db.has` first, or wrap the `get` in `s.try_catch`; don't assume a nullable result.",
    "- `db.edit` binds the **full, post-mutation row** (the freshly-written values, not the pre-edit ones). `db.del` binds the **full deleted row**. `db.add` binds the **full inserted row**, including the auto-assigned `id` and `created_at`. So `InferRow<typeof table>` is the right response type for all three.",
    "- `db.edit` and `db.del` also **throw** (404) when no row matches the field. `db.add` throws on a unique-constraint violation.",
    "",
    "Auth & calls:",
    "",
