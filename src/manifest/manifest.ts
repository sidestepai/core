...
const VALUE_CONSTRUCTORS: ReadonlyArray<ManifestValue> = [
  ...
  { name: "c.obj", signature: "(o?: Json | null) => Value", description: 'Object constant (JSON string) → tag "const:obj". No argument = the empty object {}, the editor default — use this one. Explicit null = the LEGACY BLANK form (stored value ""), which the engine evaluates to null, NOT {}; it exists so a pulled workspace round-trips exactly, so do not author it. Plain JSON literals only — a nested tagged value (inp/ref/auth/c.*) is rejected; for a computed object response use a record of values, not c.obj (issue #42).' },
