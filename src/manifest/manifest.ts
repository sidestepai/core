...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  - `schedule?`: a `ScheduleDef[]` (NOT a single object) — `{ startsOn, freq?, repeatEnabled?, endsOn? }`. `freq` is the repeat interval **in seconds** (default `86400` = daily); `endsOn` present ⇒ the schedule has an end. Deploys but does NOT fire in the sandbox (see the sandbox NOTE above).",
    ...
    "  - `output?`: `{ schema: unknown[], enabled? }` — structured-output schema (stored `structuredOutputsSchema`).",
    "",
    "### Triggers",
    "",
