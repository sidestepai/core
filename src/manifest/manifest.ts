...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  omitted `id` throws. A `system:false` PK is the author's to supply.",
  ...
  lines.push(
    ...
    "- **Statements with an `as`** also take `asFilters?` — `fl.*` filters on the RESULT as it binds, in order, same slot as `disabled`: `s.set_var(\"x\", v, { asFilters: [fl.trim(), fl.lower()] })`. Saves a follow-up `set_var`. Throws without an `as`, and does NOT retype the variable.",
