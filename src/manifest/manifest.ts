...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  config, frontend/ — with the decoded workspace filling `xano/`: one directory",
    "  per kind with each object under its parent (`query/<group>.ts` beside",
    "  `query/<group>/<name>_<VERB>.ts`, `table/trigger/`, `agent/trigger/`, and",
    "  realtime's three levels as `realtime_server/<server>.ts` beside",
    "  `<server>/<channel>.ts` beside `<channel>/<message>.ts`, with a `trigger/`",
    "  at each level). Anything holding children is a file named for ITSELF beside",
    "  their folder, so no two tabs read alike and a childless container needs no",
    "  folder. Every table lives in `table/table.ts`, settings",
    "  in `xano/workspace.ts`, a `_shared.ts` for anything else referenced from more",
    "  than one file, a barrel `xano/index.ts`, and `xano/README.md` carrying the",
    "  decode report. Paths are LOWER CASE throughout (the HTTP verb is the one",
    "  exception), while bindings keep the object's own casing. So",
    ...
    "  `xano/workspace.ts`, so treat a pulled tree as secret-bearing. Deploying it is a",
