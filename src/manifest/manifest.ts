...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "already-exported bundle instead. Auth is OAuth: run `sidestep login --instance",
    "<origin>` once (browser consent; tokens cached in project-local",
    "`.xano/auth.json`, auto-gitignored) — `push` reuses and refreshes them. Target",
    "instance via `--instance`/`$XANO_INSTANCE` (default: the cached instance).",
    "AGENTS/CI: authenticate with `$XANO_REFRESH_TOKEN` + `$XANO_CLIENT_ID` + `push`",
    "(non-interactive refresh grant; both env vars come from `.xano/auth.json`) —",
    "do NOT run `sidestep login`, which blocks on a browser consent no agent can",
    "complete. Refresh tokens rotate on use (a stored one may be single-use);",
    "`XANO_NO_BROWSER=1` suppresses the browser spawn for headless login.",
    "WARNING — this RESETS the target sandbox before loading and writes to the",
    "user's live instance; it is a transient dev workflow, NOT a permanent deploy.",
