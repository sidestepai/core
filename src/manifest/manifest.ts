...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  - `s.realtime.get_session({ as })` binds the CALLER's realtime session for the current frame: the resolved channel path, its bound path params (`ref(\"session.params.room_id\")`), and the connection's identity (who authenticated, and its extras). Only meaningful inside a realtime message or channel-trigger stack — off that path there is no session and it degrades to an anonymous one. For a path param, prefer `inp(\"room_id\")`; reach for the session when you need the CONNECTION (identity/extras), e.g. \"who is this sender\" on an anonymous-client channel.",
    ...
    "  - **Client recipe (derive, never hardcode):** `server.getUrl(baseUrl)` → `wss://<host>/ws/<canonical>` (accepts the `https://…` instance base URL and normalizes the scheme; add `{ tenant }` for a tenant instance's `/ws/<tenant>:<canonical>`), and `channel.getChannel({ room_id: 42 })` → `\"rooms/42\"`, the path that goes in a frame's `channel` field. Both throw rather than guess (a canonical is minted by `sidestep export --lock`). Auth is a bearer token passed as the websocket SUBPROTOCOL — `new WebSocket(url, token)` — and no token means an anonymous client, admitted only where `anonymousClients: true`. Frames are JSON `{ action: \"join\"|\"leave\"|\"broadcast\"|\"ack\", channel, type?: <message name>, payload?, options?, id? }`; you must `join` before you may `broadcast`, and the server's context is only ready a moment after `open` (an immediate first frame is refused). Server frames come back as `action: join` (ack, with `params`) / `message` / `replay` / `presence_full`|`presence_join`|`presence_leave` / `conversation_start`|`conversation_end` (replayed frames flagged `conversation: true`) / `error` (`payload.message`, plus `payload.code`/`retry_after` when rate limited — a per-frame refusal, not a disconnect).",
