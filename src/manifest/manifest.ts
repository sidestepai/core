...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  - `output?`: `{ schema: Record<string, input.*>, enabled? }` — structured-output schema. `schema` is a named-field record authored with the `input.*` catalog, exactly like a `defineFunction`/`query` `input:` map (the stored `structuredOutputsSchema` is the same wire shape as function inputs). e.g. `output: { schema: { priority: input.enum([\"low\",\"high\"]), summary: input.text() } }`. Byte-verified against a live-engine golden. Note: `.result` narrowing at the call site still uses `s.ai.agent.run`'s `resultShape` hint — the authored schema shapes what the model returns, not (yet) the inferred `.result` type.",
    "",
    "### Triggers",
    "",
