...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "- `s.api.microservice({ host, port?, path, method, params, headers, timeout, follow_location, as? })` — in-cluster microservice call (`mvp:microservice_request`); typed, no TLS fields, every field but `port` required. Pass the `microservice()` DEF as `host`. `port?` folds into it as `\"name:port\"`: a def exposing ONE `servicePort` resolves automatically, SEVERAL requires it. A raw `\"name:port\"` string works, unvalidated, for instance-level ones.",
