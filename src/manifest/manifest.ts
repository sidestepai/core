...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  fully offline) and share one core. The output is the scaffold `init` writes —",
    "  root package.json (dev/build/xano:export/xano:deploy), tsconfig, vite config,",
    "  frontend/ — with the decoded workspace filling `xano/`:",
    "    - One directory per kind, each object under its parent. A container is a file",
    "      named for ITSELF beside its folder, so no two tabs read alike and a childless",
    "      container needs no folder:",
    "        `query/<group>.ts` beside `query/<group>/<name>_<VERB>.ts`",
    "        `realtime_server/<server>.ts` beside `<server>/<channel>.ts` beside `<channel>/<message>.ts`",
    "        a `trigger/` at each level (`table/trigger/`, `agent/trigger/`, …)",
    "    - Every table gets its own file under `table/`; settings in `xano/workspace.ts`;",
    "      a `_shared.ts` for anything referenced from more than one file; a barrel",
    "      `xano/index.ts`; `xano/README.md` carrying the decode report.",
    "    - Paths are LOWER CASE throughout (the HTTP verb is the one exception); bindings",
    "      keep the object's own casing.",
    "    - Objects are emitted as FACTORY calls (`table({...})`, `query({...})`, …) — the",
    "      form you author by hand — so a pulled tree keeps column/input/agent",
    "      output-schema inference. Triggers no factory expresses (the realtime types, or a",
    "      non-default history) stay `{...} satisfies TriggerDef`.",
    "    - Guids are preserved verbatim, so references stay consistent. A statement the",
    "      catalog cannot model round-trips through `raw()` (`@sidestep/core/codegen`).",
    "    - An object ALREADY EMPTY upstream decodes to a def with no `stack` and is reported",
    "      as `empty-source` — faithful, not a decode failure.",
    "    - After install the tree is loaded, re-exported, and diffed against the source",
    "      bundle; a mismatch names the object and fails the command. `--no-verify` opts out,",
    "      `--no-install` leaves it unverifiable.",
    "    - `xano/` is DISPOSABLE (re-running rewrites it, no `--force` needed — the tree",
    "      carries `xano/.sidestep-codegen.json`; the rest of the project is left alone) and",
    "      SCHEMA-ONLY (no seed rows).",
    "    - ⚠ Workspace env var VALUES ride inline in `xano/workspace.ts`, so a pulled tree is",
    "      secret-bearing. Deploying it is a full replace — send it only to an ephemeral or",
    "      sandbox env, never a workspace holding data you care about.",
    "  `sidestep workspace codegen app && cd app && npm run build && npm run xano:deploy`",
    "  ships a pulled workspace to a live ephemeral URL.",
    ...
    "",
    "**Static hosting.** `sidestep deploy ./index.ts --static ./dist` ships a built frontend",
    "with the backend in one command.",
    "- Archives the directory (dependency-free gzipped tarball, 100 MB cap) and deploys it to a",
    "  static host AFTER the backend import. Output prints the backend URL, then `Static host",
    "  deployed` with the frontend `<url>` beneath.",
    "- Target follows the destination: `--dest ephemeral` puts the frontend ON THE EPHEMERAL;",
    "  `--dest sandbox` puts it on your OWN (parent) workspace, since the sandbox tenant does not",
    "  serve static hosting. (`release`, when enabled, ships both to your instance workspace.)",
    "- The workspace is resolved from your token (`auth/me`: scoped workspace guid → numeric id)",
    "  and uploaded to `/api:meta/workspace/{id}/static_host/default/build`, which auto-creates",
    "  the host and auto-deploys to `dev`.",
    "- The static step is independent and idempotent: a failure NEVER rolls back the backend. It",
    "  exits with a distinct code and a resumable message; retry with `deploy --static ./dist`.",
    "- The CLI then polls the deployed URL until the edge serves THIS build (matching the",
    "  `X-Xano-Canonical` header it stamps), so `Frontend is live` means reachable, not merely",
    "  accepted — a cold host can 503 for tens of seconds first. An unconfirmed poll is a WARNING,",
    "  never a failure. `--no-verify` skips the wait.",
    "",
    "Backend URL injection — before archiving, an inline `<script>` is inserted at the top of the",
    "build's root index.html `<head>`, assigning the backend URL to `window.XANO_HOST` and running",
    "before the app bundle. The frontend reads it at runtime with a build-time fallback, so no",
    "rebuild is needed to target an env:",
    "  const HOST = (typeof window !== 'undefined' && window.XANO_HOST) || import.meta.env.VITE_XANO_HOST;",
    "- `--static-env KEY=VALUE` (repeatable) adds your own config, exposed as `window.<KEY>` and",
    "  merged over the auto-seeded XANO_HOST.",
    "- ⚠ A static host serves these files verbatim, so everything injected is PUBLIC — base URLs",
    "  and publishable keys only, never secrets (those go in backend env, read server-side via",
    "  `env(name)`).",
    "- Injection is skipped (a warning, not a failure) when the archive has no root index.html",
    "  with a `<head>` to anchor to.",
    "- `window.XANO_HOST` is the sandbox tenant URL the APIs answer at — the value `sandbox",
    "  details` prints as `baseUrl`, NOT `profile me` (the account instance origin).",
    "- ⚠ VERIFY with a CACHE-BUSTING fetch, not a retry loop: the host caches index.html, so",
    "  re-fetching the bare URL can keep returning the OLD (pre-injection) copy and appear to hang.",
    "  Grep the BARE token — the injected line is bracket form `window[\"XANO_HOST\"]=…`, so a dot-form",
    "  match wrongly reads as \"not injected\":",
    "  `curl -s \"$URL/?nocache=$EPOCH\" | grep XANO_HOST`. (README covers the cache TTL.)",
    "",
