...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  input use `inp(\"name\")`.",
    "- **Regex filters (`regex_matches`/`regex_match`/`regex_replace`) need `/…/` delimiters.**",
    "  They are PHP `preg_match` under the hood, so a bare JS-style body (`^[^@\\s]+@...$`)",
    "  is an invalid pattern — `regex_matches` then returns `false` for *every* input, so a",
    "  precondition on it silently rejects all values (valid ones included). Delimit it:",
    "  `\"/^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$/\"|regex_matches:$email` (flags go after the close",
    "  slash, e.g. `/foo/i`). Better still, prefer a native typed input (`input.email({...})`)",
    "  over hand-rolled validation. Third-party engine behavior.",
