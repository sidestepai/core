...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "Output prints the live URLs: `Deployed: <baseUrl>` (backend) then",
    "`Static host deployed: <url>` (frontend). The frontend goes to your OWN (parent)",
  ...
  lines.push(
    ...
    "- `s.db.query({ table, where?, additionalWhere?, sort?, paging?, output?, lock?, addon?, as? })` — search; `where` is `expr(...)` / `expr[]` (ANDed) / raw `Value`. `sort` is `[{ sortBy: <col>, dir?: \"asc\"|\"desc\"|\"rand\" }]`; `paging` is `{ page?, per_page?, offset?, totals?, metadata? }` (plain numbers/flags). `where`/`additionalWhere`/`sort`/`paging`/`output` are all applied by the engine — the filter rides `context.search`, sort/paging ride `context.return.list` (#41/#34/#36). ⚠ Supplying `paging` with metadata on (the default) wraps the result in a paging envelope `{ items: Row[], curPage, nextPage, prevPage, offset, perPage, itemsReceived }` (+ `itemsTotal`/`pageTotal` when `totals:true`) instead of a bare `Row[]`; `InferResponse` reflects that. Pass `metadata:false` to keep the bare array (#58).",
    ...
    "- **Addons** enrich returned rows: `db.query`/`get`/`add`/`edit`/`patch` accept `addon: [{ addon, as, input?, output?, children? }]`. `addon` is the target addon (name or def handle); `as` is a dotted `offset.alias` destination on the row; `input` maps addon inputs (bind a parent-row column with `out(col)`); `output` restricts addon columns; `children` nests addons. An addon is a single table-bound db query (not a statement stack): author it with `addon({ name, table, where?, sort?, output: [cols], cardinality?: \"single\"|\"list\"|\"count\"|\"exists\"|\"aggregate\", input?, context? })` and register via `registerAddons([...])`: `table` auto-fills the `context.dbo` binding, `where`/`sort` (the same `expr(...)` / `[{ sortBy, dir }]` surface as `s.db.query`) encode `context.search`/`context.sort` — `where` is the predicate binding the addon to the parent row (e.g. `expr(col(\"id\"), \"=\", inp(\"user_id\"))`), `output` names the returned columns, and `cardinality` shapes the result (`context.return.type`, omitted for the `\"list\"` default). Rarer context (`eval`/`bind`/`lock`) stays raw `context` passthrough. When you attach a typed `addon({ table, output })` handle, its alias (the last `as` segment) is merged onto the row in `InferResponse` with the graft shape: `{cols}` for `single`, `{cols}[]` for `list`, `number` for `count`, `boolean` for `exists`, `unknown` for `aggregate` (its shape depends on the `group`/`eval` you supply via a raw `context.return`). An attachment-level `output` narrows an object/array graft further; a bare-name reference grafts `unknown` — narrow it at the call site. An alias that shadows an existing column on the queried table throws at build time (rename with a `_` prefix). `db.add_or_edit`/`del`/`has`/`truncate` take no `addon`.",
    "",
    "Auth & calls:",
    "",
