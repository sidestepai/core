...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  - `realtimeMessage({ name, channel, server?, guid?, description?, active?, auth?, deliverTo?, input?, middleware?, stack?, response?, history?, disabled?, tags? })` — the invocable unit (the realtime analogue of a query). `input` types the message PAYLOAD; `deliverTo?`: `\"channel\"` (default) | `\"sender\"` | `\"others\"` | `\"explicit\"` (currently delivers to nobody — the realtime send statements are not exposed yet). `server` is required only when `channel` is a bare path.",
