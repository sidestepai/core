...
const CLI_COMMANDS: readonly ManifestCliCommand[] = [
  ...
  },
  {
    ...
    flags: [
      ...
      { flag: "--static <dir>", description: "Archive a built frontend directory and deploy it to your workspace's static host (auto-deployed to dev) after the backend import." },
...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "Typical agent flow — wire the frontend to the backend at BUILD time:",
    "  1. `sandbox deploy ./index.ts`                    (ship the backend to the sandbox)",
    "  2. `BASE=$(sandbox details | jq -r .baseUrl)`     (the sandbox's backend base URL)",
    "  3. `VITE_XANO_HOST=$BASE npm run build`           (bake it in; env-var NAME is the",
    "     framework's convention, VALUE is that baseUrl)",
    "  4. `sandbox deploy ./index.ts --static ./dist`    (ship API + app together)",
    "Read the base URL from `sandbox details` (the sandbox tenant URL the APIs answer at),",
    "NOT `profile me` (the account instance origin). The sandbox is a singleton per account,",
    "so its `baseUrl` is stable — fetch once and reuse.",
    "",
