...
const VALUE_CONSTRUCTORS: ReadonlyArray<ManifestValue> = [
  ...
  { name: "env", signature: "(name: string) => Value", description: 'Read a WORKSPACE environment variable (set via workspaceConfig({ env }) or the dashboard) → `$env.NAME`. Compiles to tag "setting" with the plain name — a workspace env var IS a setting (the raw tag "env" does not resolve workspace vars). Same tag as the sys.* built-ins, which just use $-prefixed names — so env("remote_ip") reads a user var named remote_ip, not the caller IP (use sys.remoteIp()).' },
...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "",
    "- **Workspace environment variables** set a tenant's env vars through the workspace object: `workspaceConfig({ env: { STRIPE_KEY: process.env.STRIPE_KEY!, APP_BASE_URL: \"https://…\" } })`. Author them as a name→value MAP; SideStep encodes to the bundle's TOP-LEVEL `env` array (`{ name, value, market_item }`) — the location the engine imports env from (NOT the workspace object's own `env`, which the import ignores). Read a var back with `env(\"NAME\")` (→ `$env.NAME`), which compiles to tag \"setting\" with the plain name — a workspace env var IS a setting (same tag as the `sys.*` built-ins, which use $-prefixed names). Values are SECRETS: prefer sourcing from `process.env` over committing literals, and don't commit a compiled bundle with real values. Deploy sets the declared vars on the tenant; omit `env` to leave existing env untouched. The separate `settings` field stays a plain object.",
    "",
    "Auth & calls:",
    "",
