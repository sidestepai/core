...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "A **column `default` must be ASCII** — it lands in the column's DDL, which the engine",
    "stores on an ASCII-only path, so a non-ASCII default (e.g. an emoji) is rejected at export",
    "rather than 500ing at deploy (Postgres `22021 CHARACTER NOT IN REPERTOIRE`); put the value",
    "on an `input.<type>({ default })` instead, applied at runtime bind. (issue #45)",
  ...
  lines.push(
    ...
    "- `db.edit` binds the **full, post-mutation row** (the freshly-written values, not the pre-edit ones). `db.del` binds the **full deleted row**. `db.add` binds the **full inserted row**, including the auto-assigned `id` and `created_at`. So `InferRow<typeof table>` is the right response type for those three.",
    ...
    "- **`InferResponse<typeof query>`** derives an endpoint's response type (read-side round trip, no codegen). It resolves object-literal responses to those keys, and a `response: ref(\"x\")` that returns a variable bound by a top-level `db.get`/`db.query` on a `table()` to that row shape (a list → `Row[]`, a single get → `Row`; an `output: [...]` selection narrows to a `Pick`). A value reshaped by a filter/lambda, or a variable built by control flow / `set_var` / a nested function, resolves to `unknown` — declare `responseShape` on the query (e.g. `responseShape: null as InferRow<typeof t> | null`) to close it; the declaration always overrides derivation.",
    "",
    "Auth & calls:",
    "",
