...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "- **Addons** enrich returned rows: `db.query`/`get`/`add`/`edit`/`patch` accept `addon: [{ addon, as, input?, output?, children? }]`. `addon` is the target addon (name or def handle); `as` is a dotted `offset.alias` destination on the row; `input` maps addon inputs (bind a parent-row column with `out(col)`); `output` restricts addon columns; `children` nests addons. Author the addon with `addon({ name, table, output: [cols], cardinality?: \"single\"|\"list\", input?, stack? })` and register via `registerAddons([...])`: `table` auto-fills the `context.dbo` binding, `output` names the returned columns, and `cardinality:\"single\"` grafts one object (default lists an array). When you attach a typed `addon({ table, output })` handle, its alias (the last `as` segment) is merged onto the row in `InferResponse` with the graft shape (`{cols}` for single, `{cols}[]` for list); an attachment-level `output` narrows that shape further; a bare-name reference grafts `unknown` — narrow it at the call site. An alias that shadows an existing column on the queried table throws at build time (rename with a `_` prefix). `db.add_or_edit`/`del`/`has`/`truncate` take no `addon`.",
    "",
    "Auth & calls:",
    "",
