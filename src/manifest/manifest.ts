...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "- A **filtered** operand (`withFilters(...)`) works inline in a `s.conditional`/`s.while` condition,",
    "  but NOT in a `db.query`/addon `where` (rejected at export — the engine 500s; hoist it into a",
    "  prior `s.set_var` and reference the var).",
