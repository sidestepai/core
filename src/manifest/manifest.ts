...
const CLI_COMMANDS: readonly ManifestCliCommand[] = [
  ...
  },
  {
    ...
    args: "[<file> | --bundle <path>] [--format <json|multidoc>] [--path <path>|-] [--name <name>]",
    flags: [
      { flag: "--format json", description: "(default) Compile the LOCAL workspace to the JSON bundle payload (the same bytes deploy/validate send) and write a .json file; no network. Uses an explicit entry <file>/--bundle, or — with neither — a conventional entry auto-discovered under the cwd (xano/index.ts, index.ts, …), so a bare `sandbox export` works from a project root." },
    ...
    description:
      "Export a portable artifact for the workspace you're iterating on: the JSON bundle (local compile, the default) or the sandbox tenant's XanoScript multidoc (.xs, over OAuth). `--format` defaults to json; with no entry file it auto-discovers a conventional one, so a bare `sandbox export` just works. env/records/draft are unsupported (endpoint defaults).",
  },
  {
...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  which compiles the LOCAL workspace to the JSON bundle payload (the same bytes",
    "  deploy/validate send, no network). Pass an entry `<file>`/`--bundle <path>`, or",
    "  with neither it auto-discovers a conventional entry under the cwd (`xano/index.ts`,",
    "  `index.ts`, …) — so a bare `sidestep sandbox export` works from a project root.",
    "  `--format multidoc` GETs the sandbox tenant's XanoScript multidoc over OAuth and",
    "  writes a `.xs` file — it reflects the CURRENTLY DEPLOYED sandbox, so run",
    "  `sandbox deploy` first and pass no input. Output defaults to `./sandbox.<ext>`;",
    "  `--name` overrides the basename, `--path` the location, and `--path -` streams to",
    "  stdout. env/records/draft are unsupported (endpoint defaults).",
