...
const CLI_COMMANDS: readonly ManifestCliCommand[] = [
  ...
  },
  {
    ...
    description:
      "Decode a Xano bundle into a RUNNABLE project: the same scaffold `sidestep init` writes (root package.json with dev/build/xano:export/xano:deploy, tsconfig, vite config, frontend/) with the decoded workspace filling `xano/` — per-kind directories, a `_shared.ts` for tables and cross-referenced objects, a barrel `xano/index.ts`, and `xano/README.md` carrying the decode report. So `sidestep <src> codegen app && cd app && npm run build && npm run xano:deploy` ships a pulled workspace to a live ephemeral URL. Guids are preserved verbatim so references stay consistent. A statement the catalog cannot model round-trips verbatim through `raw()` from `@sidestep/core/codegen`. After install, the tree is loaded, re-exported, and diffed against the source bundle — a mismatch names the object and FAILS. `xano/` is disposable and schema-only (no seed rows): re-running rewrites it (the rest of the project is left alone) and deploying is a full replace, so send it only to an ephemeral/sandbox env. Workspace env var VALUES are carried inline in `xano/index.ts`, so treat a pulled tree as secret-bearing. The live variants are `workspace codegen`, `sandbox codegen`, and `ephemeral codegen`.",
  },
  {
...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  ephemeral display and `--expires-hours` the TTL at create time; the parent",
    "  workspace is the one your credential is bound to, never a flag.",
    "  `--bundle <path>` uploads an already-exported bundle.",
    ...
    "  your credential is bound to (no override). COMING SOON: currently prints a notice",
    ...
    "  decode a workspace into a RUNNABLE project. All four read the compressed bundle",
    "  from `workspace/{id}/export` (the file form reads a bundle already on disk,",
    "  fully offline) and share one core. The output is the same scaffold `init`",
    "  writes — root package.json (dev/build/xano:export/xano:deploy), tsconfig, vite",
    "  config, frontend/ — with the decoded workspace filling `xano/`: per-kind",
    "  directories, a `_shared.ts` for tables and cross-referenced objects, a barrel",
    "  `xano/index.ts`, and `xano/README.md` carrying the decode report. So",
    "  `sidestep workspace codegen app && cd app && npm run build && npm run xano:deploy`",
    "  ships a pulled workspace to a live ephemeral URL. Decoded objects are emitted",
    "  as FACTORY calls (`table({...})`, `query({...})`, `defineFunction({...})`) —",
    "  the same form you author by hand — so a pulled tree keeps column/input/agent",
    "  output-schema inference; triggers are the one kind still emitted as",
    "  `{...} satisfies TriggerDef`. Object guids are preserved",
    "  verbatim, so references stay consistent; a statement the catalog cannot model",
    "  round-trips verbatim through `raw()` (`@sidestep/core/codegen`). An object",
    "  ALREADY EMPTY upstream (an endpoint created but never filled in) decodes to a",
    "  def with no `stack` and is reported as `empty-source` — faithful, not a decode",
    "  failure. After the",
    "  scaffold installs, the tree is loaded, re-exported, and diffed against the",
    "  source bundle — a mismatch names the object and fails the command",
    "  (`--no-verify` opts out; `--no-install` leaves it unverifiable). `xano/` is",
    "  DISPOSABLE (re-running rewrites it and needs no `--force`, since the tree",
    "  carries `xano/.sidestep-codegen.json`; the rest of the project is left alone)",
    "  and SCHEMA-ONLY (no seed rows). Workspace env var VALUES ride inline in",
    "  `xano/index.ts`, so treat a pulled tree as secret-bearing. Deploying it is a",
    "  full replace, so send it only to an ephemeral or sandbox env — never a",
    "  workspace holding data you care about.",
    "- `sidestep workspace <details|export|codegen>` — read the REAL workspace your",
    "  credential is bound to (`details` shows which one, and which credential chose",
    "  it, before you read it). There is deliberately NO `workspace deploy` —",
    "  SideStep never writes back to your real workspace.",
    ...
    "",
    "Auth: `auth.json` holds ONE credential, discriminated by `type`. It lives in",
    "the shared `~/.sidestep/auth.json` (reused from any project) or a",
    "project-local `.xano/auth.json` (`--local`; auto-gitignored); reads prefer an",
    "existing local one, and `--config`/`$XANO_CONFIG` always wins.",
    "  • `type: \"oauth\"` — run `sidestep login` once (browser consent; you pick",
    "    the instance and workspace there). Every `deploy` reuses and refreshes it.",
    "  • `type: \"token\"` — hand-authored, for meta-API/automation use. Write it",
    "    yourself: `{ \"type\": \"token\", \"instance_base_url\": \"https://…\",",
    "    \"workspace_id\": <n>, \"meta_api_token\": \"…\" }`. No login, no refresh.",
    "Both PIN the target: the instance and numeric workspace come from the",
    "credential, and there is no instance flag and no `--workspace` override.",
    "Run `sidestep workspace details` to see which instance/workspace you are on.",
    ...
    "with `sidestep logout` (revokes the refresh token, then deletes the file).",
  ...
  lines.push(
    ...
    "  to `check_password` (which does the comparison hash itself). (issue #109)",
    "",
  );
  ...
  lines.push(
    ...
    "  - `realtimeServer({ name, guid?, description?, enabled?, canonical?, tags?, history? })` — the container. `enabled` defaults to **false** (unlike every other `enabled` in the SDK), and an enabled server with no active channel still refuses the handshake.",
    "  - `realtimeChannel({ name, server, guid?, description?, active?, input?, anonymousClients?, presence?, publish?, conversation?, delivery?, rateLimit?, tags?, history? })` — `name` is a PATH (`\"lobby\"`, `\"rooms/{room_id}\"`) and `input` types its `{param}` segments (NOT the payload) — every `{param}` MUST have a matching input or `realtimeChannel()` THROWS, the same contract a query's URL path params carry. A literal segment beats a param, so `rooms/lobby` and `rooms/{room_id}` coexist. Matching is STRICT: segment counts must be EQUAL (`rooms/{room_id}` does NOT match `rooms/42/edit`), literals are CASE-SENSITIVE, and an empty segment is REJECTED not collapsed (a leading/trailing/doubled `/` matches nothing) — which is what `getChannel()`'s throw on an empty or slash-bearing param is protecting you from. An INACTIVE channel reports the same error as a nonexistent one (deliberate, so deactivating leaks nothing). `anonymousClients` is gated TWICE — the server admits the connection, then the channel admits the join — so setting it here alone is not enough. `publish?: { who?: \"nobody\"|\"anyone\"|\"authenticated\" (default `nobody` — nobody can publish until you set it), direct? }` — `direct` (default false) allows a client to address ANOTHER CLIENT via a frame's `options.socketId`, and is checked BEFORE `who`. `conversation?: { enabled?, limit?, ttl? }` is the client-visible TRANSCRIPT replayed to a joiner (distinct from `history`, which is execution history) — `limit` DEFAULTS TO 0 AND 0 MEANS OFF, so `{ enabled: true }` alone records nothing and replays nothing, silently; `ttl` is an IDLE expiry of the WHOLE transcript refreshed by every write (an active channel never ages out; a silent one loses all of it at once), NOT a per-message age cap. `delivery?: { guarantee?: \"at_most_once\"|\"at_least_once\", perRecipient? }` — `perRecipient` is independent of the guarantee, is a NO-OP unless the channel declares a `deliver` trigger, and costs a stack PER RECIPIENT PER MESSAGE. `rateLimit?: { messagesPerMinute? }` — 0 = unlimited, checked BEFORE the handler runs, but a COST guardrail not a security control (an anonymous client is bucketed per CONNECTION so reconnecting resets it, and it fails OPEN when its store is down).",
    "  - `realtimeMessage({ name, channel, server?, guid?, description?, active?, auth?, deliverTo?, input?, middleware?, stack?, response?, history?, disabled?, tags? })` — the invocable unit (the realtime analogue of a query). `input` types the message PAYLOAD; `deliverTo?`: `\"channel\"` (default) | `\"sender\"` | `\"others\"` | `\"explicit\"` (still delivers to NOBODY — nothing selects recipients from inside a handler; `s.realtime.publish` originates an event INTO a channel and is not a substitute). Only `\"channel\"`/`\"others\"` fan out AND are written to the `conversation` transcript — a `\"sender\"` response is invisible to every future joiner. `server` is required only when `channel` is a bare path.",
    "  - **Reading both input surfaces in the stack:** the message payload AND the channel's path params arrive as ordinary declared inputs — `inp(\"body\")` for a payload field, `inp(\"room_id\")` for the channel's `{room_id}`. Nothing else is needed (no session lookup, no frame parsing). A path param is bound ONCE at join and read from the connection thereafter, never from the frame, so a sender cannot claim a room it did not join; the same values reach a channel `join`/`leave` trigger's stack.",
    "  - `s.realtime.get_session({ as })` binds the CALLER's realtime session for the current frame. FLAT shape: `authenticated` (bool), `client_id` (text — the AUTHED ROW ID as text, `\"\"` anonymous), `dbo_id` (int, 0 anonymous), `socket_id` (int — the transport id), `channel` (text — resolved path, `\"\"` in a server trigger), `params` (object — bound path params, `{}` when none, e.g. `ref(\"session.params.room_id\")`), `extras` (object), `opened_at` (decimal). Works in a realtime MESSAGE stack and in CHANNEL and SERVER trigger stacks — off that path there is no session and it degrades to an anonymous one. For a path param prefer `inp(\"room_id\")`; reach for the session when you need the CONNECTION (identity/extras), e.g. \"who is this sender\" on an anonymous-client channel. THREE UNRELATED THINGS ARE CALLED A CLIENT ID: `session.client_id` (app-facing identity), `session.socket_id` (transport), and a frame's `options.client_id` (the at_least_once CURSOR handle) — conflating the first and last breaks at_least_once for anonymous clients.",
    "  - `s.realtime.publish({ server, channel, data, message?, authTable?, authId? })` — the PUSH direction: originate a server-authored event onto a channel from ANY stack, no client frame first. `server` is the handle or its NAME (resolved by name, not guid); `channel` is the FILLED-IN path (`channel.getChannel({ room_id: 42 })`), never the template. Three properties, all of which surprise people: DELIVERY-ONLY — fanned out as-is, does NOT invoke a `realtimeMessage()` handler even when `message` names one (a channel `deliver` trigger still runs); SERVER-AUTHORITATIVE — bypasses `publish.who`, which governs CLIENTS, so authorize in your own stack; FAIL-SOFT — a missing/disabled server or dead bus is swallowed engine-side, so a mis-targeted publish is SILENT with no result to check (only `server`/`channel` throw, at author time). `authTable`/`authId` are ASSERTED attribution on the frame — not a credential, nothing validates them.",
    "  - **Client recipe (derive, never hardcode):** `server.getUrl(baseUrl)` → `wss://<host>/ws/<canonical>` (accepts the `https://…` instance base URL and normalizes the scheme; add `{ tenant }` for a tenant instance's `/ws/<tenant>:<canonical>`), and `channel.getChannel({ room_id: 42 })` → `\"rooms/42\"`, the path that goes in a frame's `channel` field. Both throw rather than guess (a canonical is minted by `sidestep export --lock`). Auth is a bearer token passed as the websocket SUBPROTOCOL — `new WebSocket(url, token)` — and no token means an anonymous client, admitted only where `anonymousClients: true`. Frames are JSON `{ action: \"join\"|\"leave\"|\"broadcast\"|\"ack\"|\"ping\"|\"presence\", channel, type?: <message name>, payload?, options?, id? }`; you must `join` before you may `broadcast`, and the server's context is only ready a moment after `open` (an immediate first frame is refused). `options` is `{ socketId?, client_id?, channel? }` — `socketId` addresses another client directly (needs `publish.direct`), `client_id` is the at-least-once cursor handle, `options.channel` WINS over a top-level `channel`. KEEP THE SOCKET ALIVE: an idle connection is REAPED after ~10 minutes, so a LISTEN-ONLY client (feed, dashboard, presence sidebar — anything that joins and rarely publishes) MUST send `{ action: \"ping\" }` (answered `pong`) or any frame periodically, or it silently drops and reconnects forever. Server frames come back as `action: join` (ack, `{ joined: true, params }` + `cursor`/`resumed` on at_least_once) / `message` / `replay` / `broadcast` (a RECEIPT to the sender — `payload.delivered_local` counts recipients on the ANSWERING NODE ONLY, NOT the channel, so it is not a delivery confirmation; plus `id` on at_least_once and `dropped: true` when the handler returned null) / `presence_full`|`presence_join`|`presence_leave` / `conversation_start`|`conversation_end` (replayed frames flagged `conversation: true`) / `pong` / `ack` / `error` (`payload.message`, plus `code`/`limit`/`retry_after` when rate limited — `rate_limited` is the ONLY code, so do NOT switch on `code`). An `error` is a per-frame refusal, NOT a disconnect — EXCEPT a failed handshake and a REFUSED `connect` TRIGGER, which each send one and then CLOSE with code 4401.",
    "  - **Tenant instances (isolated DB):** a tenant's realtime objects live in the TENANT's database, so BOTH halves of a client must name the tenant. Socket: `server.getUrl(base, { tenant })` → `/ws/<tenant>:<canonical>` — the tier splits on the FIRST `:`, applies the tenant DB, and only THEN resolves the canonical, so a bare canonical on a tenant host is looked up in the instance workspace instead (a miss, or a different workspace's channels). That colon form is PECULIAR TO THE SOCKET — one segment, tenant glued to canonical — while every other tenant URL gives the tenant its OWN segment: the HTTP half of the same client is `https://<host>/tenant/<tenant>/api:<canonical>/…`. NO request header is required for either. Because the shapes differ, `getUrl` TRANSLATES a tenant base URL instead of concatenating: pass the `https://<host>/tenant/<name>` that `sandbox details` prints (and that deploy injects as `window.XANO_HOST`) and the tenant is LIFTED into the socket form — `getUrl(window.XANO_HOST)` needs no `{ tenant }` at all — while a CONFLICTING `{ tenant }` alongside it throws. Still pass `{ tenant }` explicitly for a tenant on its OWN DOMAIN: HTTP resolves that one by hostname, but the websocket tier reads only the connection hash, so there is nothing in the URL to lift. Tokens are tenant-scoped (audience `<tenant>:<license>`, not the bare license), so one minted through the instance workspace is REJECTED by a tenant's realtime server — authenticate and dial through the same tenant, or the socket fails auth for a reason nothing on the wire explains. `/ws` itself is the instance ingress's routing segment, stripped before the tier reads the rest of the path as the connection hash.",
    "  - **Presence frames** (a `presence: true` channel only): `presence_full` carries `payload.members` — an ARRAY holding the WHOLE roster including the receiving client — and `presence_join`/`presence_leave` carry a single `payload.member`. A member is `{ id, dbo_id, authenticated, extras, joined_at }`: `id` the auth row id as a string (`\"\"` anonymous), `dbo_id` the auth table's id (`0` anonymous), `extras` the connection's extras object, `joined_at` epoch SECONDS. Render from `presence_full`, then apply the deltas — the roster counts MEMBERS, not connections (refcounted per identity, so a user's second tab fires no second `presence_join`). Join order is `join` ack → `presence_full` → (others get `presence_join`) → conversation replay → `replay` frames, and a joined client can re-request the snapshot any time with `{ action: \"presence\", channel }` (answered to the SENDER only; a socket that never joined is REFUSED, so the roster is not readable without membership).",
    "  - **Conversation frames — the transcript hydrates the client, so DO NOT build a hydration endpoint.** On a `conversation: { enabled: true }` channel the replay is PUSHED automatically at join, unasked: `conversation_start` (`payload.count`) → the last `limit` messages, each a normal `action: \"message\"` frame carrying its ORIGINAL `type` and `payload` plus `conversation: true` and the original `ts` → `conversation_end`. The client needs NO fetch, no `GET /messages`, and no table read to paint the initial view — render `message` frames the same way whether or not `conversation` is set, and the backfill paints itself. BUT `{ enabled: true }` ALONE IS A NO-OP — `limit` defaults to 0 and 0 means RETAIN NONE (not retain everything), so the transcript is never written and never replayed, with no error. ALWAYS PASS `limit`. Three consequences: the POST-HANDLER broadcast payload IS the stored transcript row, so a handler must broadcast everything the UI needs to render a past message (author name, id, `created_at`) — nothing else is replayed; only `deliverTo` `\"channel\"`/`\"others\"` are RECORDED, so a `\"sender\"` response is invisible to every future joiner by construction; and the transcript is a capped ring (`limit`, `ttl`), not storage, so persist to a table only when you need durability, search, or reads BEYOND that window (audit, moderation, paging older than `limit`) — never merely to hydrate a joiner.",
    "  - **`delivery.guarantee: \"at_least_once\"` is a CLIENT CONTRACT, not just a channel setting.** The client must ACK what it receives — `{ action: \"ack\", channel, id }`, confirmed by `{ action: \"ack\", channel, payload: { cursor } }` — and an ANONYMOUS client must ALSO send a durable `options.client_id` in its JOIN frame (once; later acks need not repeat it). WITHOUT one it has no cursor, its acks are SILENTLY IGNORED, and it degrades to at_most_once — the guarantee you configured is simply absent with nothing on the wire to say so. An AUTHENTICATED client is keyed by identity and needs no `client_id`. The missed gap arrives after join as `replay` frames (oldest-first, each with an `id` to ack) — DISTINCT from the conversation transcript: `conversation_*` is the SHARED \"what was said before I arrived\", `replay` is the PER-CLIENT \"what I missed while disconnected\"; both may be on. How far back it reaches is sized by `conversation.ttl` (here a REAL per-message age cut, and it BEATS `limit`), else `conversation.limit`, else 1000 — even on a channel with no transcript enabled.",
    "  - **What a message handler RETURNS decides delivery, and the failure directions are NOT symmetric.** A returned value fans out per `deliverTo` and becomes the transcript row. Returning NULL delivers NOTHING — the supported way for a handler to veto its own message (the sender is told `dropped: true`). A payload REJECTED by the declared `input` also delivers nothing, and the detail goes ONLY to the sender. But a handler that CRASHES FAILS OPEN: the sender's ORIGINAL, UNVALIDATED payload is broadcast to the channel unchanged (so one workspace bug cannot black-hole a channel) — so a handler doing redaction or authorization must NOT be the only thing between client input and subscribers.",
    "",
    "### Triggers",
    "",
