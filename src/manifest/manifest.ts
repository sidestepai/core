...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  per kind with each object under its parent (`query/<api group>/<name>_<VERB>.ts`,",
    "  `table/trigger/`, `agent/trigger/`, and realtime's three levels as",
    "  `realtime_server/<server>/<channel>/<message>.ts` with a `trigger/` at each",
    "  level), every table in `table/table.ts`, settings",
