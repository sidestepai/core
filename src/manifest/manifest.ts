...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "type is nullable. Engine-side; a geo column is write-and-forget for now. `s.db.edit` on a",
    "row holding one also fails, so keep geo on an insert-only side table.",
