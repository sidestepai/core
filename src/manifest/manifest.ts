...
const VALUE_CONSTRUCTORS: ReadonlyArray<ManifestValue> = [
  ...
  { name: "c.blank", signature: '(tag: "const:int" | "const:decimal" | "const:array" | "const:expr" | …) => Value', description: 'The editor\'s UNCONFIGURED value box — a cell added and never filled in, stored as value "". Emitted by codegen for a pulled workspace that holds one; do not author it. It is NOT a zero, an empty string, or an empty collection — the engine reads "" and "0" differently, so c.blank("const:int") and c.int(0) are different stored values and neither is canonicalized into the other. Constant tags only (c.text("") and c.obj(null) already spell the other two blanks).' },
...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  output-schema inference. Triggers no factory expresses (the realtime types, or",
    "  a non-default history) stay `{...} satisfies TriggerDef`. Object guids are preserved",
