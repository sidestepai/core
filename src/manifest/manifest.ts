...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  Fix: `sidestep routes <entry> --emit xano/routes.gen.ts`",
    "  emits every verb + resolved path as plain data importing NOTHING, still compile-checked",
    "  (`routePath(\"blog/{slug}\", { slug })`) — a rename is a type error, not a 404.",
  ...
  lines.push(
    ...
    "**`f.geo.*` takes WKT TEXT on write, never GeoJSON.** A `{ type, coordinates }` value is",
    "refused (`400 Only strings are supported for: POINT`). Build it with",
    "`geo({ type: \"Point\", coordinates: [lng, lat] })`, which converts to WKT and refuses a",
    "wrong-arity position or an unclosed polygon ring by name; or pass raw WKT",
    "(`c.text(\"POINT(1 2)\")`). Coordinates are longitude-FIRST. ⚠ Reading a geo column back",
    "currently returns `null` on every path (`db.get`/`db.query`, with or without `output`) —",
    "the `{ type, data }` shape appears only in the echo a `db.add` returns, so its declared",
    "type is nullable. The value IS stored — but the engine writes it in a form its own reader",
    "rejects, which shows up three ways: reads return null, `s.db.edit` on ANY column of a row",
    "holding one fails its WKT validator, and a workspace export with records writes `null`, so",
    "**export → re-import silently loses every geo value** (the schema survives). All engine-",
    "side. Until it is fixed, treat a geo column as write-and-forget, keep it on an insert-only",
    "side table, and do not rely on it surviving a backup, clone or branch copy.",
