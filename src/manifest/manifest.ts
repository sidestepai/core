...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "- **Addons** enrich returned rows: `db.query`/`get`/`add`/`edit`/`patch` accept `addon: [{ addon, as, input?, output?, children? }]`. `addon` is the target addon (name or def handle); `as` is a dotted `offset.alias` destination on the row; `input` maps addon inputs (bind a parent-row column with `out(col)`); `output` restricts addon columns; `children` nests addons. Addon-added fields are not yet reflected in `InferResponse`. `db.add_or_edit`/`del`/`has`/`truncate` take no `addon`.",
    "",
    "Auth & calls:",
    "",
