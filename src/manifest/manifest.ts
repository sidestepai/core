...
const CLI_COMMANDS: readonly ManifestCliCommand[] = [
  ...
  },
  {
    ...
    flags: [
      ...
      { flag: "--static <dir>", description: "Archive a built frontend directory and deploy it to the sandbox's static host after the backend import." },
      { flag: "--config <path>", description: "Project-local token cache (default: $XANO_CONFIG, then ./.xano/auth.json)." },
    ],
    description:
      "Deploy the compiled workspace to your sandbox (replaces the removed `push`), optionally with a static frontend. The sandbox is the only deploy target. Never writes SERVER identities back into xano.lock (the compile step still maintains it exactly as `export` does).",
  },
  {
    command: "profile me",
    args: "[--config <path>]",
...
const VALUE_CONSTRUCTORS: ReadonlyArray<ManifestValue> = [
  ...
  { name: "c.obj", signature: "(o: Record<string, unknown>) => Value", description: 'Object constant (JSON string) → tag "const:obj".' },
  { name: "c.array", signature: "(a: unknown[]) => Value", description: 'Array constant (JSON string) → tag "const:array".' },
  { name: "ref", signature: "(name: string) => Value", description: 'Reference a stack variable → tag "var".' },
...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "  BASE URL as JSON. An agent reads `instance` to bake the frontend's API base",
    "  into its config BEFORE building + uploading `--static`.",
    "",
    ...
    "it to the sandbox's static host AFTER the backend import. No separate frontend",
    "host to configure, no CI upload script — backend and static frontend ship",
    "together. Output prints the live URLs: `Deployed: <baseUrl>` (backend) then",
    "`Static host deployed: <url>` (frontend). The sandbox has no static-host route",
    "of its own, so the CLI reaches one through an impersonation hop",
    "(`sandbox/impersonate` → `tenant/token/exchange`) and uploads to the ordinary",
    "static-host build endpoint with the returned `X-Tenant` credentials. The static",
    "step runs only after the backend import commits (it needs the sandbox workspace",
    "id from that response) and is independent and idempotent: a static failure never",
    "rolls back the backend — it exits with a distinct code and a resumable message,",
    "and you retry just the static step by re-running `sandbox deploy --static ./dist`.",
    "Typical agent flow: read the instance URL with `profile me` → build the frontend",
    "against it → `sandbox deploy --static ./dist` to ship API + app together.",
    "",
  ...
  lines.push(
    ...
    "positional payload before opts. `input.*` fully mirrors `f.*` — every type below is",
  ...
  lines.push(
    ...
    "- `s.db.query({ table, where?, additionalWhere?, sort?, paging?, output?, lock?, as? })` — search; `where` is `expr(...)` / `expr[]` (ANDed) / raw `Value`.",
