...
export function renderLlmsTxt(m: Manifest): string {
  ...
  lines.push(
    ...
    "does NOT expose: the engine names tables `x<workspace_id>_<table_id>` (e.g. `x6_203970`),",
    "ids assigned at import — not knowable from a `table()` def (name + guid only), and",
    "`sql_name` persists empty. So the safe counter drops out of the typed surface: hardcode",
